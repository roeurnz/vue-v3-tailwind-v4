<template>
  <aside class="bg-card border-r border-border p-4 z-0 overflow-hidden h-full" :class="[
    draggedWidth === 0 ? 'w-0 overflow-hidden' : (isCollapsed ? 'w-16' : 'w-64'),
    { 'select-none': isResizing }
  ]" :style="[
      draggedWidth === 0 ? { width: '0px', padding: '0px' } : { width: draggedWidth + 'px' },
      isResizing ? { 'transition': 'none', 'willChange': 'width' } : { 'transition': 'all 0.2s ease-in-out' }
    ]">
    <!-- Header with Toggle Menu -->
    <div class="flex items-center justify-between mb-4" :class="[
      draggedWidth < 200 ? 'opacity-60' : 'opacity-100',
      isResizing ? '' : 'transition-all duration-300 ease-in-out'
    ]" v-if="!isCollapsed && draggedWidth > 0">
      <h3 class="text-sm font-medium" :class="[
        draggedWidth < 150 ? 'text-xs' : 'text-sm',
    ...
    <!-- Collapsed Header (Simplified) -->
    <div class="flex justify-center mb-4" :class="[
    ...
    <div class="absolute right-0 top-0 bottom-0 w-2 cursor-col-resize z-20"
      style="background: transparent; pointer-events: auto; transform: translateZ(0);" @mousedown="startResize"
      @dblclick="toggleCollapse" :title="$t('resizeSidebar')"></div>


    <!-- Floating Expand Button (when completely hidden) -->
    <div v-if="draggedWidth === 0" class="fixed left-0 top-1/2 transform -translate-y-1/2 z-50">
      <button @click="restoreSidebar"
    ...
    <nav class="space-y-1" v-if="!hideApps && draggedWidth > 0">
      <router-link to="/" class="flex items-center rounded hover:bg-accent text-sm" :class="[
        getMenuItemClasses(),
        isResizing ? '' : 'transition-all duration-200'
      ...
      <router-link to="/solutions" class="flex items-center rounded hover:bg-accent text-sm" :class="[
        getMenuItemClasses(),
        isResizing ? '' : 'transition-all duration-200'
      ...
      <router-link to="/tools" class="flex items-center rounded hover:bg-accent text-sm" :class="[
        getMenuItemClasses(),
        isResizing ? '' : 'transition-all duration-200'
      ...
      <router-link to="/about" class="flex items-center rounded hover:bg-accent text-sm" :class="[
        getMenuItemClasses(),
        isResizing ? '' : 'transition-all duration-200'
    ...
    <div class="mt-8" :class="isResizing ? '' : 'transition-all duration-300 ease-in-out'"
      v-if="!hideFolders && !isCollapsed && draggedWidth > 0">
      ...
      <ul class="space-y-1 text-sm">
        <li><a href="#"
            class="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-all duration-200 rounded px-1 py-1 hover:bg-accent/50"
            :class="draggedWidth < 120 ? 'justify-center px-1' : 'justify-start px-2'">
            <div class="w-3 h-3 bg-red-500 rounded flex-shrink-0"></div>
            <span v-if="draggedWidth >= 120" class="truncate">{{ $t('work') }}</span>
          </a></li>
        <li><a href="#"
            class="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-all duration-200 rounded px-1 py-1 hover:bg-accent/50"
            :class="draggedWidth < 120 ? 'justify-center px-1' : 'justify-start px-2'">
            <div class="w-3 h-3 bg-blue-500 rounded flex-shrink-0"></div>
            <span v-if="draggedWidth >= 120" class="truncate">{{ $t('personal') }}</span>
          </a></li>
        <li><a href="#"
            class="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-all duration-200 rounded px-1 py-1 hover:bg-accent/50"
            :class="draggedWidth < 120 ? 'justify-center px-1' : 'justify-start px-2'">
            <div class="w-3 h-3 bg-green-500 rounded flex-shrink-0"></div>
            <span v-if="draggedWidth >= 120" class="truncate">{{ $t('important') }}</span>
          </a></li>
      </ul>
    </div>

    <!-- Bottom Controls (Collapse/Expand Buttons) -->
    <div class="mt-auto pt-4 border-t border-border"
      :class="isResizing ? '' : 'transition-all duration-300 ease-in-out'" v-if="draggedWidth > 0">
      <div class="flex items-center justify-between">
        <span class="text-xs text-muted-foreground" v-if="draggedWidth >= 120 && !isCollapsed">{{ $t('sidebarControls')
          }}</span>
        <button @click="toggleCollapse" class="p-2 hover:bg-accent rounded transition-colors"
          :title="isCollapsed ? $t('expandSidebar') : $t('collapseSidebar')">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              :d="isCollapsed ? 'M13 5l7 7-7 7M5 5l7 7-7 7' : 'M11 19l-7-7 7-7m8 14l-7-7 7-7'"></path>
          </svg>
        </button>
      </div>
    </div>
